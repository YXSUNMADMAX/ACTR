...
class Evaluator:
    ...
    @classmethod
    def eval_kps_transfer_it(cls, prd_kps, batch, idx):
        r"""Compute percentage of correct key-points (PCK) based on prediction"""

        pk, tk = prd_kps[idx], batch["trg_kps"][idx]
        thres = batch["pckthres"][idx]
        npt = batch["n_pts"][idx]
        _, correct_ids, _ = cls.classify_prd(pk[:, :npt], tk[:, :npt], thres)
        pck = (len(correct_ids) / npt.item()) * 100
        return [pck]
    ...
    def intersection_over_union(cls, mask1, mask2):
        ...
        rel_part_weight = torch.sum(
            torch.sum(mask2.gt(0.5).float(), 2, True), 3, True
    ...
    def ptstr2mask(cls, x_str, y_str, out_h, out_w):
        ...
        mask = (
            torch.tensor(mask_np.astype(np.float32)).unsqueeze(0).unsqueeze(0).float()
        )
